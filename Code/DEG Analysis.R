...
BiocManager::install(c("DESeq2","RUVSeq", "GEOquery","Biobase"))
...
#Transformation
msk <- estimateSizeFactors(dds)
vsd <- varianceStabilizingTransformation(dds, blind = FALSE)

#Scatter Plots Comparison
par(mfrow=c(1, 2))
lims <- c(-2, 20)
plot(log2(counts(msk, normalized = TRUE)[,1:2] + 1), pch=16, cex=0.3, main="log2(x + 1)", xlim=lims, ylim=lims)
...
#Assigning Gene Stats for Volcano Plots
res_nhbe$stat <- ifelse(res_nhbe$padj < 0.05 & abs(res_nhbe$log2FoldChange) > 1,
                        ifelse(res_nhbe$log2FoldChange > 0, "Upregulated", "Down-Regulated"), 
                        "Non-Significant")

res_a549$stat <- ifelse(res_a549$padj < 0.05 & abs(res_a549$log2FoldChange) > 1,
                        ifelse(res_a549$log2FoldChange > 0, "Upregulated", "Down-Regulated"), 
                        "Non-Significant")

res_a549_ace2$stat <- ifelse(res_a549_ace2$padj < 0.05 & abs(res_a549_ace2$log2FoldChange) > 1,
                        ifelse(res_a549_ace2$log2FoldChange > 0, "Upregulated", "Down-Regulated"), 
                        "Non-Significant")

res_calu3$stat <- ifelse(res_calu3$padj < 0.05 & abs(res_calu3$log2FoldChange) > 1,
                        ifelse(res_calu3$log2FoldChange > 0, "Upregulated", "Down-Regulated"), 
                        "Non-Significant")

#Volcano Plots
volcano_colors <- c("Upregulated" = "#4DAF4A", "Down-Regulated" = "#E41A1C", "Non-Significant" = "#377EB8")
ggplot(res_nhbe,
  aes(x = log2FoldChange, y = -log10(padj), color = stat)) +
...
sig_genes_nhbe <- as.data.frame(res_nhbe[res_nhbe$padj < 0.05 & abs(res_nhbe$log2FoldChange) > 1, ])
up_genes_nhbe <- subset(sig_genes_nhbe, log2FoldChange > 0)
down_genes_nhbe <- subset(sig_genes_nhbe, log2FoldChange < 0)

sig_genes_a549 <- as.data.frame(res_a549[res_a549$padj < 0.05 & abs(res_a549$log2FoldChange) > 1, ])
up_genes_a549 <- subset(sig_genes_a549, log2FoldChange > 0)
down_genes_a549 <- subset(sig_genes_a549, log2FoldChange < 0)

sig_genes_a549_ace2 <- as.data.frame(res_a549_ace2[res_a549_ace2$padj < 0.05 & abs(res_a549_ace2$log2FoldChange) > 1, ])
...
sig_genes_calu3 <- as.data.frame(res_calu3[res_calu3$padj < 0.05 & abs(res_calu3$log2FoldChange) > 1, ])
...
top_up_nhbe <- head(up_genes_nhbe[order(up_genes_nhbe$log2FoldChange, decreasing = TRUE), ], 10)
...
top_down_nhbe <- head(down_genes_nhbe[order(down_genes_nhbe$log2FoldChange, decreasing = FALSE), ], 10)
...
top_up_a549 <- head(up_genes_a549[order(up_genes_a549$log2FoldChange, decreasing = TRUE), ], 10)
...
top_down_a549 <- head(down_genes_a549[order(down_genes_a549$log2FoldChange, decreasing = FALSE), ], 10)
...
top_up_a549_ace2 <- head(up_genes_a549_ace2[order(up_genes_a549_ace2$log2FoldChange, decreasing = TRUE), ], 10)
...
top_down_a549_ace2 <- head(down_genes_a549_ace2[order(down_genes_a549_ace2$log2FoldChange, decreasing = FALSE), ], 10)
...
top_up_calu3 <- head(up_genes_calu3[order(up_genes_calu3$log2FoldChange, decreasing = TRUE), ], 10)
...
top_down_calu3 <- head(down_genes_calu3[order(down_genes_calu3$log2FoldChange, decreasing = FALSE), ], 10)
...
sig_genes_nhbe$stat <- ifelse(sig_genes_nhbe$log2FoldChange > 0, "Upregulated", "Downregulated")
sig_genes_nhbe$gene_id <- rownames(sig_genes_nhbe)
sig_genes_nhbe <- sig_genes_nhbe[, c(7, 1:6)]
write.table(sig_genes_nhbe, file = "result/significant_DE_NHBE_genes.csv", sep = ",", row.names = FALSE, quote = FALSE)

sig_genes_a549$stat <- ifelse(sig_genes_a549$log2FoldChange > 0, "Upregulated", "Downregulated")
sig_genes_a549$gene_id <- rownames(sig_genes_a549)
sig_genes_a549 <- sig_genes_a549[, c(7, 1:6)]
write.table(sig_genes_a549, file = "result/significant_DE_A549_genes.csv", sep = ",", row.names = FALSE, quote = FALSE)

sig_genes_a549_ace2$stat <- ifelse(sig_genes_a549_ace2$log2FoldChange > 0, "Upregulated", "Downregulated")
sig_genes_a549_ace2$gene_id <- rownames(sig_genes_a549_ace2)
sig_genes_a549_ace2 <- sig_genes_a549_ace2[, c(7, 1:6)]
write.table(sig_genes_a549_ace2, file = "result/significant_DE_A549_ACE2_genes.csv", sep = ",", row.names = FALSE, quote = FALSE)

sig_genes_calu3$stat <- ifelse(sig_genes_calu3$log2FoldChange > 0, "Upregulated", "Downregulated")
sig_genes_calu3$gene_id <- rownames(sig_genes_calu3)
sig_genes_calu3 <- sig_genes_calu3[, c(7, 1:6)]
